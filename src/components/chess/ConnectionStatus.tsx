...
export function ConnectionStatus() {
  ...
  const handleConnect = async () => {
    ...
    toast({
      title: "Connecting...",
      description: `Attempting to connect to the Render.com chess server`
    });
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center">
          ...
          <span className="text-sm text-yellow-500">
            {connecting || reconnecting ? 
              "Connecting to Render.com chess server..." : 
              "Not connected to the chess server. Click connect to establish a connection."}
          </span>
        </div>
        {!connecting && !reconnecting && (
